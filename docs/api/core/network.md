...
```ruby
network = RobotLab.create_network(name: "support", run_config: config) do
...
```ruby
network.task(name, robot, context: {}, mcp: :none, tools: :none, memory: nil, run_config: nil, depends_on: :none)
# => self
```
...
| `run_config` | `RunConfig`, `nil` | Per-task config (merged on top of network's RunConfig) |
...
```ruby
{
  ...
  run_config: { model: "claude-sonnet-4", temperature: 0.7 }  # if set
}
```
